...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    for epoch in range(2):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/pro/5_fold_roc_pro_{}.pth".format(fold)
            if not os.path.exists("outputs/pro"):
                os.makedirs("outputs/pro")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/5_fold_roc_protbert.png',dpi=400)
