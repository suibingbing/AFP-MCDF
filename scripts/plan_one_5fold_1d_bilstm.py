...
class MyModel(nn.Module):
    def __init__(self,):
        ...
        self.fc_class1 = nn.Linear(512, 2)
...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    for epoch in range(2):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/1db/5_fold_roc_1db_{}.pth".format(fold)
            if not os.path.exists("outputs/1db"):
                os.makedirs("outputs/1db")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/1db/5_fold_roc_1db.png',dpi=400)
