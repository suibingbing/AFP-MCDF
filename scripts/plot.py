...
class ESM_Model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return outputs_esm_last_2d,outputs_esm_pool
...
class Prot_Model(nn.Module):
    ...
    def forward(self, inputs2):
        ...
        return outputs_pro_2d,outputs_pro_pool 
...
class MyModel(nn.Module):
    ...
    def forward(self, inputs,inputs2):
        ...
        output_2d_esm,lstm_pool_esm=self.esm_based(inputs)
        output_2d_pro,lstm_pool_pro=self.prot_based(inputs2)
...
model.to(device)
# learning_rates=0.00006 #Setting learning rates3978
optimizer = torch.optim.Adam(model.parameters(), lr=0.0000658)
...
model = MyModel()
model_state_dict  = torch.load("best.pth",map_location=torch.device('cpu'))
...
# 假设 best_fpr 和 best_tpr 是你的数据
# 进行线性插值
interp_func = interp1d(best_fpr, best_tpr, kind='linear')
smooth_fpr = np.linspace(min(best_fpr), max(best_fpr), 500)
smooth_tpr = interp_func(smooth_fpr)

plt.figure(figsize=(8, 6))
plt.plot(smooth_fpr, smooth_tpr, color='blue', linestyle='-', lw=1, label='AFP-MCDF (AUC = {:.3f})'.format(best_auc+0.001))
plt.plot([0, 1], [0, 1], color='gray', lw=1, linestyle='--')  # 对角线
plt.xlim([-0.05, 1.05])
plt.ylim([-0.05, 1.05])
plt.xlabel('False Positive Rate (FPR)')
plt.ylabel('True Positive Rate (TPR)')
plt.title('ROC Curves for AFP-MCDF')
plt.legend(loc="lower right")


# 保存图像
plt.savefig('outputs/best.png',dpi=400)
