...
class MyModel(nn.Module):
    ...
    def forward(self, inputs,inputs2):
        ...
        # x = x.view(8, -1)
        # x=self.fc_class1(x)
        two_d=lstm_pool_esm+lstm_pool_pro
...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    for epoch in range(2):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/2dt/5_fold_roc_2dt_{}.pth".format(fold)
            if not os.path.exists("outputs/2dt"):
                os.makedirs("outputs/2dt")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/2dt/5_fold_roc_2dt.png',dpi=400)
