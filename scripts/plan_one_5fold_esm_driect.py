...
class MyModel(nn.Module):
    def __init__(self,):
        ...
        self.fc_class1 = nn.Linear(192000, 2)
    ...
    def forward(self, inputs,inputs2):
        ...
        x=self.esm_based(inputs)
        x = x.view(8, -1)
        # lstm_pool_esm=self.prot_based(inputs2)
        x=self.fc_class1(x)
...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    model = MyModel()#Model loading
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.00001)# 模型训练
    item=0
    for epoch in range(1):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/esm_drie/5_fold_roc_esm_drie_{}.pth".format(fold)
            if not os.path.exists("outputs/esm_drie"):
                os.makedirs("outputs/esm_drie")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/esm_drie/5_fold_roc_esm_drie.png',dpi=400)
