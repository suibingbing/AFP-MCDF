...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    model = MyModel()#Model loading
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0000660)# 模型训练
    item=0
    for epoch in range(10):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/lr/5_fold_roc_0.0000660_{}.pth".format(fold)
            if not os.path.exists("outputs/lr"):
                os.makedirs("outputs/lr")
            torch.save(model.state_dict(), file_path)
