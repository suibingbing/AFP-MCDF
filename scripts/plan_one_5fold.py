...
optimizer = torch.optim.Adam(model.parameters(), lr=0.0000675)
...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    model = MyModel()
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0000665)# 模型训练
    item=0
    for epoch in range(10):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/5_fold_roc_r1_{}.pth".format(fold)
            # if not os.path.exists("outputs/5fold_esm2_12_lstm_200"):
            #     os.makedirs("outputs/5fold_esm2_12_lstm_200")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/5_fold_roc_r1.png',dpi=400)
