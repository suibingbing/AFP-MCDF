...
class MyModel(nn.Module):
    def __init__(self,):
        ...
        self.fc_class1 = nn.Linear(409600, 2)
...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    model = MyModel()#Model loading
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0000035)# 模型训练
    item=0
    for epoch in range(1):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/pro_driec/5_fold_roc_pro_driec_{}.pth".format(fold)
            if not os.path.exists("outputs/pro_driec"):
                os.makedirs("outputs/pro_driec")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/5_fold_roc_protbert_direct.png',dpi=400)
