...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    for epoch in range(2):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/2d/5_fold_roc_2d_{}.pth".format(fold)
            if not os.path.exists("outputs/2d"):
                os.makedirs("outputs/2d")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/2d/5_fold_roc_2d.png',dpi=400)
