...
class MyModel(nn.Module):
    def __init__(self,):
        ...
        self.fc_class1 = nn.Linear(480, 2)
    ...
    def forward(self, inputs,inputs2):
        ...
        x=self.fc_class1(lstm_pool_esm)
...
for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
    ...
    model = MyModel()#Model loading
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.00005)# 模型训练
    item=0
    for epoch in range(1):
        ...
        if auc>best_auc:
            ...
            best_tpr=tpr
            best_auc=auc
            file_path = "outputs/esm/5_fold_roc_esm_{}.pth".format(fold)
            if not os.path.exists("outputs/esm"):
                os.makedirs("outputs/esm")
            torch.save(model.state_dict(), file_path)
...
plt.savefig('outputs/5_fold_roc_esm.png',dpi=400)
